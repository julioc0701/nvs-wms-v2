...
export default function Picking() {
  ...
  async function handleShortageConfirm(qtyFound, notes) {
    ...
    // Nota: NÃO chamamos api.reportShortage aqui — o endpoint /tiny/shortages
    // já consolida automaticamente via PickingItem.shortage_qty. Dupla escrita
    // gerava registros fantasma sem marketplace.
  ...
  async function _doOutOfStock(notes) {
    ...
    // Nota: NÃO chamamos api.reportShortage aqui — o endpoint /tiny/shortages
    // já consolida via PickingItem.shortage_qty. Dupla escrita gerava registros
    // fantasma sem marketplace.
    setSession(prev => prev ? { ...prev, progress: res.progress } : prev)
    if (res.item?.labels_ready) await autoPrintLabels(res.item)
    if (focusSku) goBackToItems()
    else {
