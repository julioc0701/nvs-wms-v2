...
from financeiro_ml.router import router as financeiro_ml_router
import financeiro_ml.models as _fml_models  # noqa: F401
...
app.include_router(ml_full_plans.router, prefix="/api/ml-full-plans", tags=["ml-full-plans"])
app.include_router(financeiro.router, prefix="/api/financeiro", tags=["financeiro"])
app.include_router(financeiro_ml_router, prefix="/api/financeiro-ml", tags=["financeiro-ml"])
...
def sync_v2():
    ...
    repo_db = os.path.abspath(os.path.join(os.path.dirname(__file__), "warehouse_v3_local.db"))
